...
def resize_image_list(list_image_merge):
    ...
    for image in list_image_merge:
        ...
        w_min = min(w_min,w_temp)
        # print(w_temp,h_temp)
    # print(w_max)
    for image in list_image_merge:
        wpercent = (w_min / float(image.size[0]))
        hsize = int((float(image.size[1]) * float(wpercent)))
        image_resize = image.resize((w_min, hsize), Image.ANTIALIAS)
...
if __name__ == "__main__":
    ...
    for data in datasets:
        ...
        img_label = add_x_label(np.array(list_image_merge[0]),label=label,font_size=font_size_dict[data]) # dnd
    ...
    label = [r'Noise',r'N2V',r'Laine19' ,r'N2N', r'Ours',r'Clean']
